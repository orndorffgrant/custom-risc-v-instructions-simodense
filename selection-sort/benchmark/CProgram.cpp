...
void selection_sort_chunks(uint32_t* dest, uint32_t* src, size_t len);
...
void main(int argc, char** argv) {
	...
	//volatile uint64_t time00, time0, time1, time2, time3_, time3;
	...
	for (size_t len=16*4; len<=1024/*33554432*/*4; len*=2){
		...
		uint64_t icount2=insn()-icount1;  
	  	...
	  	// Print result	
	  	printf("MergeSIMDSort results:     cycle count: %llu, instruction count: %llu, CPI: %llu.%02llu, ",
	  	time2, icount2,(time2)/(icount2), 	(((time2)%(icount2))*100)/(icount2));
	  	
	  	uint64_t int_part = (len*150)/time2;
	  	uint64_t dec_part = (((len*150)%time2)*100)/time2;
	  	printf("%llu.%02llu MB/s@150MHz\n",int_part,dec_part);
		

		// Read cycles and instruction count	  		
		uint64_t time00=time(); 
		uint64_t icount00=insn();
		
		// Two phase selection-merge sort
		selection_sort_chunks(srcC, srcC, len);
		mergeSort(srcC,0,len/4);

		// Read cycles and instruction count	
		uint64_t time0=time()-time00;
		uint64_t icount0=insn()-icount00;  
		  		  	
	  	// Print result	
	  	printf("SelectionSIMDSort results: cycle count: %llu, instruction count: %llu, CPI: %llu.%02llu, ",
	  	time0, icount0,(time0)/(icount0), 	(((time0)%(icount0))*100)/(icount0));
	  	
	  	uint64_t int_part0 = (len*150)/time0;
	  	uint64_t dec_part0 = (((len*150)%time0)*100)/time0;
	  	printf("%llu.%02llu MB/s@150MHz\n",int_part0,dec_part0);


		// Do the same for qsort()
		volatile uint64_t time3_=time(); 
		uint64_t icount3_=insn();
		qsort (srcB, (len/4), sizeof(int), comp);
		uint64_t time3=time()-time3_;
		uint64_t icount3=insn()-icount3_;
		
	  	uint64_t int_part2 = (len*150)/time3;
	  	uint64_t dec_part2 = (((len*150)%time3)*100)/time3;
		// Print result
		printf("QSort results:             cycle count: %llu, instruction count: %llu, CPI: %llu.%02llu, %llu.%02llu MB/s@150MHz\n",
		time3, icount3, time3/icount3, ((time3%icount3)*100)/icount3,
		int_part2, dec_part2);



		printf("Speedup from MergeSIMDSort over QSort:     %llu.%02llu\n", time3/time2, ((time3%time2)*100)/time2);
		printf("Speedup from SelectionSIMDSort over QSort: %llu.%02llu\n", time3/time0, ((time3%time0)*100)/time0);
		//printf("Speedup from MergeSIMDSort over SelectionSIMDSort: %llu.%02llu\n", time0/time2, ((time0%time2)*100)/time0);
...
{ 
  ...
  for (int i=0; i<len; i+=incr*2){
	// Load vectors to v1 and v2
  	...
  	asm volatile ("c3 x0, x0, %0":: "I"(v1));
  	asm volatile ("c3 x0, x0, %0":: "I"(v2));
